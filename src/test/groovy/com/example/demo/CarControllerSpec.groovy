...
class CarControllerSpec extends Specification{
    ...
    def "Get by ID - REST"() {
        given: "an entity (location saved as it is a required field in Car)"
            ...
            def c1 = new Car(name:'Fast Car', registration:'BA9 4TY', manufacturer:'Ferrari',model:'No Idea', transmission:'AUTOMATIC', category:'A',location:l1_post)
    ...
    def "Get all cars - REST"() {
        given: "multiple cars are posted (and their relevant locations)"
            def l1_post = locationRepository.save(l1)
            def l2_post = locationRepository.save(l2)
            def c1 = new Car(name:'Fast Car', registration:'BA9 4TY', manufacturer:'Ferrari',model:'No Idea', transmission:'AUTOMATIC', category:'A',location:l1_post)
            def c2 = new Car(name:'Slow Car', registration: 'MP9 JS7', manufacturer: "Me", model: 'XD24', transmission: 'MANUAL', category: 'D', location:l1_post)
            def c3 = new Car([name:'Super Fast Car', registration:'XDD 900', manufacturer:'Irarref', model:'Onifotrop', transmission:'MANUAL', category:'S', location:l2_post])
    ...
    def "Post car - REST"() {
        when: "car is added (with loc)"
            def l1_post = locationRepository.save(l1)
            def c1 = new Car(name:'Fast Car', registration:'BA9 4TY', manufacturer:'Ferrari',model:'No Idea', transmission:'AUTOMATIC', category:'A',location:l1_post)
            def c1JsonString = mapper.writeValueAsString(c1)

            def e1 = mockMvc.perform(MockMvcRequestBuilders
                    .post("/api/cars").contentType(MediaType.APPLICATION_JSON).content(c1JsonString))
                    .andExpect(MockMvcResultMatchers.status().isOk())
                    .andReturn().response.contentAsString
            def e1id = new JsonSlurper().parseText(e1)
        then: "check name matches"
            def result = mockMvc.perform(get("/api/cars/$e1id")).andReturn().response.contentAsString
            def json = new JsonSlurper().parseText(result)
            json.id == e1id
         and:
            json.location.id == l1_post.getId()
    }
